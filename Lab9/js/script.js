...
async function vota() {
  ...
  session_id = await fetch('https://api.themoviedb.org/3/authentication/guest_session/new', options)
  .then(response => response.json())
  .then(response => session_id = response.guest_session_id)
  .catch(err => console.error(err)); 
  ...
  const opzioni = {
    ...
    headers: {
      ...
      Authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJjYTdkYjExNDIxZDNjN2EyMzJkMjI3MDBmODMzYWQwOSIsInN1YiI6IjY1NmYzNjY4NjUxN2Q2MDE1MTY3MDljMiIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.cDWflphlNzGZuIYUpsdLUdA7rupNt0jQvDZ-S8-9UvI'
    },
    body : JSON.stringify(payload)
  };
  
  fetch('https://api.themoviedb.org/3/movie/' + id + '/rating', opzioni)
    .then(response => response.json())
    .then(response => {
      document.getElementById('alertVoto').classList.remove('d-none');
    })
    .catch(err => console.error(err));
}